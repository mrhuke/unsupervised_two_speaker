...
gammaToneFilter::gammaToneFilter(float centerF, float bC, int sf)
{
	cf = centerF;
	bw = 24.7 * ( cf*0.00437 + 1.0) * bC;

	float d = 1.5/PI/bw * float(sf);
	if (fmod(d, 1)>=0.5) delay=floor(d)+1;
	else delay=floor(d);
	delay=0;

	twoPiT = 2*PI/float(sf);
...
void gammaToneFilter::oneStep(float input)
{
	float x[4], y[4];
...
void gammaToneFilter::filtering(float *input, int sigLength, float *response)
{
	int n, k;
...
gammaToneFilterBank::gammaToneFilterBank(gammaTonePara x)
{
	...
	float lowerERB = HzToERBRate(x.lCF);
	float upperERB = HzToERBRate(x.rCF);

	float spaceERB = (x.nChan > 1) ? (upperERB-lowerERB)/(x.nChan-1) : 0;

	for (int chan=0; chan<x.nChan; chan++)
	{
		float cf = ERBRateToHz(lowerERB + chan*spaceERB);
...
void gammaToneFilterBank::filtering(float *input, int sigLength, int chan)
{
	if (memIndi[chan]==1) delete [] response[chan];
	response[chan] = new float[sigLength];
	memIndi[chan]=1;
