function sig = ibmNorm(target,masker,snr,lc,fRange)
...
if nargin < 2
    ...
    sig = synthesisNorm(target,ones(numChan,numFrame));  % equivalent to an all-one mask
    return
end
if nargin < 3
...
gt = gammatoneNorm(target,numChan,fRange);
gm = gammatoneNorm(masker,numChan,fRange);
...
sig = synthesisNorm(mixture,mask,fRange);
