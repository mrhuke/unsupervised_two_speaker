...
temp1 = gammatone_Ke(mixture, numChan, fRange, fs);          % first pass through gammatone filterbank
...
for c = 1:numChan
    ...
    gt(c,:) = 1/3*(2*pi*b(c)/fs).^4*fs^3*tmp_t.^(filterOrder-1).*exp(-2*pi*b(c)*tmp_t).*cos(2*pi*cf(c)*tmp_t+phase(c));
end

% ke_gammatone response normalization
if length(cf)>=10
...
for kk=1:numChan
    gtf(kk,:) = (1/3*(2*pi)^4)*3*2*1*1/2*(b(kk)./sqrt((2*pi*(f-cf(kk))).^2+(2*pi*b(kk))^2)).^filterOrder;
    gtf(kk,:) = gtf(kk,:).^2;
end
...
for c = 1:numChan
    ...
    temp1(c,:) = fliplr(temp1(c,:));    % time reverse filter output & normalize out mid-ear coefficients
    ...
    temp1(c,:) = fliplr(temp2(1:sigLength));    % time reverse again & normalize out mid-ear coefficients
...
% show summed square frquency reponse of gammatone filters
f_gt = zeros(numChan,gL);
for kk=1:numChan
    dummy = fft(gt(kk,:));
    f_gt(kk,:) = dummy.*conj(dummy);
end
figure; plot(sum(f_gt));